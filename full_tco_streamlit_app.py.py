...
st.success(
    "Full TCO results are loaded. Sidebar input changes require clicking **Run Full TCO Model**. "
...
# ============================================================
# STACKED FULL TCO PLOT
# ============================================================
def make_full_tco_stacked_plot(metric: str):
    if metric == "$/mile":
    ...
    fig, axes = plt.subplots(
        1,
        len(apps_to_plot),
        figsize=(6.3 * len(apps_to_plot), 6.0)
    )
    ...
    return fig, file_label


st.subheader("Full TCO Component Stacked Plot")
st.markdown(
    "Each stacked segment shows the P50 cost component. The black error bar shows total Full TCO P5–P95. "
    "Application titles show whether economic inputs are Default or Customized."
)

fig_full, file_label = make_full_tco_stacked_plot(metric_display)
st.pyplot(fig_full)

buf_full = BytesIO()
fig_full.savefig(buf_full, format="png", dpi=600, bbox_inches="tight")
buf_full.seek(0)

st.download_button(
    label=f"Download Full TCO stacked plot PNG, 600 dpi ({metric_display})",
    data=buf_full,
    file_name=f"full_tco_stacked_{file_label}.png",
    mime="image/png",
)
